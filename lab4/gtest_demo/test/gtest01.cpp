...
// 测试用例1：测试程序能否根据指定的目录获取文件名
TEST(IntegrationTest, TestGetFileNames) {

    InputFolder inFolder("../input");
    EXPECT_EQ(inFolder.folderPath, "test_folder");
    EXPECT_EQ(inFolder.ls.size(), 2); // 假设inFolder目录下有2个子文件夹

    output o;
    o.init();
    // 假设equal.csv、inequal.csv和abnormal.csv文件创建成功
    EXPECT_TRUE(o.equal.is_open());
    EXPECT_TRUE(o.inequal.is_open());
    EXPECT_TRUE(o.abnormal.is_open());

    EXPECT_EQ(inFolder.ls[0].filenames[0], "../gtest_demo/filepath/file1.txt");
    EXPECT_EQ(inFolder.ls[0].filenames[1], "../gtest_demo/filepath/file1.txt");
    EXPECT_EQ(inFolder.ls[1].filenames[0], "../gtest_demo/filepath/file1.txt");
    EXPECT_EQ(inFolder.ls[1].filenames[1], "../gtest_demo/filepath/file1.txt");
    for (int i =0;i<inFolder.ls.size();i++)
    {
        ...
        // testAll()函数会依次测试所有的文件对，并将结果写入output对象
        // 因此可以通过检查output对象中记录的结果来验证testAll()函数的正确性
//    std::vector<std::pair<std::string, std::string>> equals = o.getEqualList();
//    std::vector<std::pair<std::string, std::string>> inequals = o.getInequalList();

    }
    o.writeAb();
//    std::vector<std::pair<std::string, std::string>>abnormal = o.getInequalList();

    EXPECT_EQ(o.abnormalList[0], "../gtest_demo/filepath/file1.txt");
    EXPECT_EQ(o.abnormalList[1], "../gtest_demo/filepath/file1.txt");
    EXPECT_EQ(o.abnormalList[2], "../gtest_demo/filepath/file1.txt");
}

// 测试用例2：测试程序在非法路径下能否正常工作
TEST(IntegrationTest, TestGetFileNamesWithInvalidPath) {
...
// 测试用例3：测试程序能否对指定目录下的不同文件类型进行测试
TEST(IntegrationTest, TestGetFileNamesWithMultipleFileTypes) {
    string path = "../gtest_demo/filepath";
    vector<string> filenames;
